...
class PShare:
    def __init__(self, n, m):
        ...
        self.local_history_table = [0] * (2**m)
        self.prediction_table = [[0] * (2**n) for _ in range(2**m)]

    def predict(self, address, outcome):
        global_history_idx = self.global_history & ((1 << self.m) - 1)
        local_history_idx = address & ((1 << self.m) - 1)
        local_history = self.local_history_table[local_history_idx]

        prediction_idx = (global_history_idx << self.n) | local_history
        ...
        self.global_history = ((self.global_history << 1) & ((1 << self.m) - 1)) | (outcome == "T")
    ...
    def make_prediction(self, address, branch_is_taken):
        ...
        if branch_is_taken:
            ...
            if p > 0:
                self.prediction_table[global_history][index] -= 1
        self.global_history = ((self.global_history << 1) + branch_is_taken) & ((1 << self.m) - 1)
        return p >= 2**(self.m - 1)
...
class Tournament(BranchPredictor):
    ...
    def __init__(self, m: int, n: int) -> None:
        """
        Initializes the PShare and GShare predictors, along with the prediction table for the Tournament predictor.
        """
        self.n = n
        self.m = m
        self.pshare_predictor = PShare(n, m)
        self.gshare_predictor = GShare(n, m)
        self.prediction_table = [[0, 0] for _ in range(2**m)]

    def make_prediction(self, address: int, branch_is_taken: bool) -> bool:

        """
        Make the prediction using both the PShare and GShare predictors, and update the prediction table accordingly.
        """
        pshare_prediction = self.pshare_predictor.make_prediction(address, branch_is_taken)
        gshare_prediction = self.gshare_predictor.make_prediction(address, branch_is_taken)

        tournament_prediction = self.prediction_table[self.gshare_predictor.branch_history][1]

        if pshare_prediction != gshare_prediction:
            if self.pshare_predictor.make_prediction(address, branch_is_taken):
                tournament_prediction = self.prediction_table[self.gshare_predictor.branch_history][0]
            else:
                tournament_prediction = self.prediction_table[self.gshare_predictor.branch_history][1]

        """
        Update the meta predictor history table accordingly.
        """
        if (pshare_prediction != branch_is_taken) and (gshare_prediction == branch_is_taken):
            self.prediction_table[self.gshare_predictor.branch_history][0] = max(self.prediction_table[self.gshare_predictor.branch_history][0] - 1, 0)
            self.prediction_table[self.gshare_predictor.branch_history][1] = min(self.prediction_table[self.gshare_predictor.branch_history][1] + 1, 3)

        if (pshare_prediction == branch_is_taken) and (gshare_prediction != branch_is_taken):
            self.prediction_table[self.gshare_predictor.branch_history][0] = min(self.prediction_table[self.gshare_predictor.branch_history][0] + 1, 3)
            self.prediction_table[self.gshare_predictor.branch_history][1] = max(self.prediction_table[self.gshare_predictor.branch_history][1] - 1, 0)

        self.gshare_predictor.update_history(branch_is_taken)

        return tournament_prediction >= 2
