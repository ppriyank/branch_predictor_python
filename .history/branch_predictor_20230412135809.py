...
class Tournament(BranchPredictor):
    ...
    """
    def __init__(self, m: int, n: int) -> None:
        self.gshare_predictor = GShare(m, n)
        self.pshare_predictor = PShare(m, n)
        self.choice_predictor = BranchPredictor(m, counter_bits=2)

    def make_prediction(self, address: int, branch_is_taken: bool) -> bool:
        gshare_prediction = self.gshare_predictor.predict_taken(address)
        pshare_prediction = self.pshare_predictor.make_prediction(address, branch_is_taken)
        choice_prediction = self.choice_predictor.predict_taken(address)
        
        if choice_prediction:
            self.gshare_predictor.make_prediction(address, branch_is_taken)
            return gshare_prediction
        else:
            self.pshare_predictor.make_prediction(address, branch_is_taken)
