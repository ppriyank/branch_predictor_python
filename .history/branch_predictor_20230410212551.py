...
class PShare:
    def __init__(self, m1, m2):
        ...
        self.local_history_table = [0] * (2**m2)
        self.prediction_table = [[0] * (2**m1) for _ in range(2**m2)]

    def predict(self, address, outcome):
        global_history_idx = self.global_history & ((1 << self.m2) - 1)
        local_history_idx = address & ((1 << self.m2) - 1)
        ...
        if outcome == "T":
            prediction = min(prediction + 1, (1 << self.m1) - 1)
        else:
        ...
        self.local_history_table[local_history_idx] = ((local_history << 1) & ((1 << self.m1) - 1)) | (outcome == "T")

        return prediction >= (1 << (self.m1 - 1))
    
    def make_prediction(self, address, branch_is_taken):
        index = address & ((1 << self.m1) - 1)
        ...
        if branch_is_taken:
            ...
            if p > 0:
                self.prediction_table[global_history][index] -= 1
        self.global_history = ((self.global_history << 1) + branch_is_taken) & ((1 << self.m2) - 1)
